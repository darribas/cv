...
//
// Any record's optional `links` array (official version, code, data, a talk's
// video, …, mirrored from me.darribas.org) — a publication here, or any
// cv.json entry above — is deliberately NOT read anywhere in this file: those
// extras belong on the web page, not in a standard academic PDF. Omission IS
// the mechanism — a renderer shows a field only if its template names one
// (ARCHITECTURE.md, "Web-only / PDF-extended fields"). An extended PDF variant
// would opt in via a `sys.inputs` flag; nothing to gate until such a variant
// exists.
